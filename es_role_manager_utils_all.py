...
class ElasticsearchRoleManager:
    ...
    def analyze_kibana_privileges(
        ...
        """
        # Extract all spaces the role has access to
        spaces = ElasticsearchRoleManager.extract_kibana_spaces(role_definition)
        
        if not spaces:
            return False, set(), set()
        
        # Get existing privileges for these spaces
        existing_privileges = ElasticsearchRoleManager.get_existing_kibana_privileges(role_definition, spaces)
        
        # Find spaces that need privilege updates
        spaces_needing_update = set()
        all_missing_privileges = set()
        
        for space in spaces:
            space_privileges = existing_privileges.get(space, set())
    ...
    def add_kibana_privileges_to_role(
        ...
        When adding .all privileges, this method removes superseded privileges (.read, .minimal_read, 
        and sub-privileges like .generate_report, .url_create) to avoid conflicts.
        ...
        # e.g., if adding feature_discover.all, we need to remove feature_discover.read, etc.
        features_getting_all = set()
        for priv in privileges:
            if priv.endswith('.all'):
                # Extract feature name (e.g., 'feature_discover' from 'feature_discover.all')
        ...
        for app_entry in updated_role['applications']:
            ...
            if app_entry.get('application', '').startswith('kibana'):
                ...
                if missing_privileges:
                    ...
                    # When adding feature_X.all, remove feature_X.read, feature_X.minimal_read,
                    # feature_X.generate_report, feature_X.url_create, feature_X.download_csv_report, etc.
                    cleaned_privileges = set()
                    removed_privileges = set()
                    
                    for existing_priv in existing_privileges:
                        # Check if this privilege is superseded by an incoming .all privilege
                        is_superseded = False
                    ...
                    if removed_privileges:
                        self.logger.debug(
                            f"Removed superseded privileges: {removed_privileges} from entry with resources: {app_entry.get('resources', [])}"
                        )
                    self.logger.debug(
                        f"Added {len(missing_privileges)} privileges to entry with resources: {app_entry.get('resources', [])}"
                    )
