...
class ElasticsearchRoleManager:
    ...
    def analyze_kibana_privileges(
        ...
        Entries with base space privileges (space_read, space_all) are excluded from analysis
        since mixing them with feature-level privileges causes Kibana UI issues.
        ...
        """
        # Base space privileges that are incompatible with feature-level privileges in Kibana UI
        base_space_privileges = {'space_read', 'space_all'}
        
        # Extract all spaces the role has access to, excluding those with base privileges
        spaces = set()
        spaces_with_base_privs = set()
        ...
        for app_entry in role_definition.get('applications', []):
            if app_entry.get('application', '').startswith('kibana'):
                ...
                for resource in app_entry.get('resources', []):
                    if resource.startswith('space:') or resource == '*':
                        # Check if this entry has base space privileges
                        if entry_privileges & base_space_privileges:
                            spaces_with_base_privs.add(resource)
                        else:
                            spaces.add(resource)
        ...
        # Get existing privileges for these spaces (only from entries without base privileges)
        privileges_by_space = {space: set() for space in spaces}
    ...
    def add_kibana_privileges_to_role(
        ...
        Entries with base space privileges (space_read, space_all) are skipped since mixing
        them with feature-level privileges causes Kibana UI issues.
        ...
        for app_entry in updated_role['applications']:
            ...
            if app_entry.get('application', '').startswith('kibana'):
                ...
                # Skip entries with base space privileges (space_read, space_all)
                # Mixing these with feature-level privileges causes Kibana UI issues
                if existing_privileges & base_space_privileges:
                    entries_skipped += 1
                    self.logger.debug(
                        f"Skipping entry with base space privileges: {existing_privileges & base_space_privileges} "
        ...
        self.logger.debug(f"Updated {entries_updated} Kibana application entries, skipped {entries_skipped} with base space privileges")
