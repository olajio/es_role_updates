...
Usage:
    ...
    python es_role_auto_update.py --config config.json --role-file roles.txt \
        --remote-clusters prod --ccs-cluster ccs

    # Only add patterns to remote clusters (no CCS sync)
    python es_role_auto_update.py --config config.json --roles role1 \
        --remote-clusters prod qa dev --skip-ccs
"""
...
def main():
    ...
    try:
        ...
        if not total_remote_updates and not ccs_updates:
            logger.info("\n✓ No roles need updating. All roles are up to date.")
            print_summary(
                remote_updates, ccs_updates, {}, {},
                args.dry_run, args.skip_remote, args.skip_ccs,
                remote_inject_patterns, ccs_inject_patterns,
        ...
        generate_report(
            remote_updates, ccs_updates, report_file,
            remote_inject_patterns, ccs_inject_patterns,
            remote_clusters, ccs_cluster or "N/A"
        )
        ...
        if args.report_only:
            ...
            print_summary(
                remote_updates, ccs_updates, {}, {},
                True, args.skip_remote, args.skip_ccs,
                remote_inject_patterns, ccs_inject_patterns,
                remote_clusters, ccs_cluster or "N/A"
            )
            return 0
